...
namespace WickedCrush
{
    ...
    public class Level
    {
        ...
        /*public void loadLevel(String LEVEL_NAME)
        {
            ...
            List<Block> blockList = new List<Block>(); //all the blocks used in the level
            List<Block> completeBlockList = new List<Block>(); //all the blocks in the game directory

            List<Tile> tileList = new List<Tile>(); //all the tiles used in the level
            List<Tile> completeTileList = new List<Tile>(); //all the tiles used in the level
            ...
            getCompleteBlockList(completeBlockList);
            getCompleteTileList(completeTileList);
            ...
            foreach (XElement e in grid.Elements("coord"))
            {
                ...
                elementName = e.Attribute("block").Value;
                sGCD[row, col] = new Platform(col * gridSize, row * gridSize, gridSize, gridSize, blockSelection(elementName, completeBlockList));
                blockList.Add(blockSelection(elementName, completeBlockList));
            }

            foreach (XElement e in background.Elements("coord"))
            {
                col = int.Parse(e.Attribute("x").Value);
                row = int.Parse(e.Attribute("y").Value);
                elementName = e.Attribute("tile").Value;
                levelBackgrounds[row, col] = elementName;
                tileList.Add(tileSelection(elementName, completeTileList));
            }

            foreach (XElement e in foreground.Elements("coord"))
            {
                col = int.Parse(e.Attribute("x").Value);
                row = int.Parse(e.Attribute("y").Value);
                elementName = e.Attribute("tile").Value;
                levelForegrounds[row, col] = elementName;
                tileList.Add(tileSelection(elementName, completeTileList));
            }

            blockList = removeDupesFromBlockList(blockList);
            tileList = removeDupesFromTileList(tileList);

            texList = getTextureList(blockList, tileList, _overlord._cm);
            normList = getNormalList(blockList, tileList, _overlord._cm);

            solidGeomTex = compileTex(_overlord._cm, _overlord._gd, texList, out texLookupList, 128);
            solidGeomNorm = compileTex(_overlord._cm, _overlord._gd, normList, out normLookupList, 128);

            createSolidGeomVertices(blockList, tileList);
            ...
            cf.AddLightToList(new GlowingPointLight(hero,
                ...
                new Vector4(0.6f, 0.6f, 0.8f, 0.5f),
                500f,
                0.5f,
                new Vector2(40f, 50f),
                0.035f,
        ...
        public void loadLevel(String LEVEL_NAME)
        {
            ...
            //OptimizePlatforms(completeMatList);
        ...
        /*
        public void levelChanged(ContentManager cm, GraphicsDevice gd)
        {
            List<Texture2D> texList, normList;
            List<Block> blockList = new List<Block>(); //all the blocks used in the level
            List<Block> completeBlockList = new List<Block>(); //all the blocks in the game directory
            ...
            for(int i = 0; i < sGCD.GetLength(0); i++)
            {
                ...
                {
                    if (sGCD[i, j] != null)
                        blockList.Add(blockSelection(sGCD[i, j].blockName, completeBlockList));
                    if (levelBackgrounds[i, j] != null)
                        tileList.Add(tileSelection(levelBackgrounds[i, j], completeTileList));
                    if (levelForegrounds[i, j] != null)
                        tileList.Add(tileSelection(levelForegrounds[i, j], completeTileList));
                }
            }
            ...
            texList = getTextureList(blockList, tileList, cm);
            normList = getNormalList(blockList, tileList, cm);

            solidGeomTex = compileTex(cm, gd, texList, out texLookupList, 128);
            solidGeomNorm = compileTex(cm, gd, normList, out normLookupList, 256);

            createSolidGeomVertices(blockList, tileList);

            if (solidGeomVertices.Length > 0)
            {
                levelVertexBuffer = new VertexBuffer(gd, typeof(VertexPositionNormalTextureTangentBinormal), solidGeomVertices.Length, BufferUsage.None);
                levelVertexBuffer.SetData(solidGeomVertices);
            }
        }
        */
        ...
        /*
        public void ContentLostFix(ContentManager cm, GraphicsDevice gd) //this shit
        {
            List<Texture2D> texList, normList;
            List<Block> blockList = new List<Block>(); //all the blocks used in the level
            List<Block> completeBlockList = new List<Block>(); //all the blocks in the game directory
        ...
        */
        ...
        //you are my greatest failure, but you worked exactly as i intended:
        /*private void OptimizePlatforms(List<Material> matList) // wait, this works? WOW WOW WOW WOW
            //works slower than goint directly to correct unoptimized
        {
            int[,] temp = new int[sGCD.GetLength(0), sGCD.GetLength(1)];
            Point start, size;
            String possiblyDepricated;
            int i, j, k, l;
            //bool finished = false;
            bool working = false;
            bool snagged = false;

            solidGeom = new List<Platform>();
                ...
                for (i = 0; i < temp.GetLength(0); i++)
                    for (j = 0; j < temp.GetLength(1); j++)
                    {
                        if (temp[i, j] == 1)
                        {
                            start.X = i;
                            start.Y = j;
                            possiblyDepricated = sGCD[i, j].matName;
                            ...
                            while (working)
                            {
                                ...
                                for (k = i; k < i + size.X; k++)
                                {
                                    ...
                                    if (l >= temp.GetLength(1))
                                        working = false;
                                
                            }

                            solidGeom.Add(new Platform(start.Y*gridSize, start.X*gridSize, size.Y * gridSize, size.X * gridSize, matSelection(possiblyDepricated, matList)));

                            for (k = start.X; k < size.X; k++)
                                for (l = start.Y; l < size.Y; l++)
                                    temp[k, l] = 0;
